...
## アプリの基本情報
QR/バーコードをスキャンするメインの機能に加えて、QRコードを作成するサブ機能を備えたアプリです。<br>
リアルタイムでQR/バーコードをスキャンするシーンでは、直感的なUI/UXを意識しています。

過去のスキャン結果を確認するシーンでは、目的の情報へのアクセスのしやすさ、検索しやすさを重視しました。<br>
また、QRコードを作成する機能は、よりスムーズに情報を入力できること、いち早く作成したQRコードを共有できる仕組みとしています。

![introduction](images/introduction.png)

## 開発背景

市場には既に多くのQRコードスキャンアプリが存在します。<br>
しかし、ダウンロード数が5億を超えるアプリであっても、実際に利用すると不便さや、もどかしさを感じることが少なからずあります。

当アプリは、それらの不便さを参考に私ならどう作るかを示すことを目的として開発しました。
...
| 2 | 一覧表示のアイテムを見分けにくい | アイテムがURL形式の場合、タイトルをhttps://〜としているアプリが存在するが、複数のURL形式のアイテムが並んだ場合に見分けにくさを感じた。<br>日常生活でURL形式のQRをスキャンする頻度は、他の形式のQRよりも多いため改善が必要と感じた。（店舗紹介や商品紹介、飲食店のメニューなど） | URLからサイトのページ名、ファビコンを取得して表示する。 |
...
| 4 | 詳細画面の表示 | スキャン結果のデータを羅列しただけのアプリが散見される。<br>例えば、連絡先形式のデータの場合、複数の電話番号、メールアドレスが並ぶこともあるが、せめて自宅、勤務先などのラベルの表示が欲しいところ。 | ・詳細表示の各項目に項目名を表示する<br>・データの羅列ではなく表形式で整理して表示する |
