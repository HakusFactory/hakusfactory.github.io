...
| 8 | タブレット端末などの大型デバイス対応 | アダプティブなUI/UXを実現するため、アダプティブナビゲーションと ListDetailPaneScaffold を採用しました。<br><br>当アプリでは独自のナビゲーションバーを表示していますが、タブレット端末などの大型デバイスではナビゲーションバーの表示が間延びする傾向がありました。<br>そのため、アダプティブナビゲーションを用いて、大型デバイスではナビゲーションバーではなくナビゲーションレールを表示する構成にしました。<br><br>また、履歴・お気に入り・検索機能においては、コンテンツの比較のしやすさ、戻る操作の削減、モダンな印象の提供を目的に、ListDetailPaneScaffold により一覧と詳細を同時表示するUIを実現しています。 |
